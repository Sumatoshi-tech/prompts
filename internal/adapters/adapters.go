...
func skillsForWorkflow(workflow string) map[string]SkillDef {
	...
	for k, v := range baseSkills {
		result[k] = v
	}

	if wfSkills, ok := workflowSkills[workflow]; ok {
		for k, v := range wfSkills {
			result[k] = v
		}
	} else {
		// Default to FRD workflow.
		for k, v := range workflowSkills[config.WorkflowFRD] {
			result[k] = v
		}
	}
...
func placeClaude(skills []SkillDef, rendered map[string][]byte) ([]PlacedFile, error) {
